...
export const updateLeaveStatus = async (req, res) => {
	try {
	  ...
	  // Update status dan rejectionReason
	  ...
	  if (status === "Ditolak") {
		...
		updateFields.push("rejectionReason = ?");
		updateValues.push("-"); // Reset rejectionReason kalau Approve
	  }
	  ...
	  if (status === "Disetujui") {
		...
		const [absensiCuti] = await pool.query(
		  `SELECT * FROM Kehadiran WHERE employeeID = ? AND tanggal IN (${placeholders}) AND status = 'Cuti'`,
		  [employeeID, ...formattedDates]
		);
  
		if (absensiCuti.length > 0) {
		  await pool.query(
			`DELETE FROM Kehadiran WHERE employeeID = ? AND tanggal IN (${placeholders}) AND status = 'Cuti'`,
			[employeeID, ...formattedDates]
		  );
		}
	  }
