...
router.get("/", authenticate, async (req, res) => {
    try {
        ...
        if (req.user.departmentID === 1) {
            // HR: ambil semua
            fetchUrl = "http://localhost:3000/api/leaves";
        } else {
            // User biasa: hanya ambil leave milik dia sendiri
            fetchUrl = `http://localhost:3000/api/leaves/employee/${req.user.id}`;
        }
        ...
        const leaveRequests = Array.isArray(apiLeaves) ? apiLeaves.map((leave) => ({
            id: leave.id || leave.leaveID,
            employee: leave.employee || leave.employee_name,
            type: leave.type || (leave.keterangan_Cuti?.includes('Sakit') ? 'Sick' : 'Personal'),
