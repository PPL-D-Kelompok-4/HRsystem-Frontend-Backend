document.addEventListener("DOMContentLoaded", () => {
	...
	function showPage(page) {
		...
		employeeCountText.textContent = `Showing ${Math.min(
			start + 1,
			rows.length
		)} - ${Math.min(end, rows.length)} of ${rows.length} employees`;
	}
	...
	document.addEventListener("click", (event) => {
		const isClickInsideMenuButton = event.target.closest(
			"[id^='employeeMenuButton-']"
		);
		...
		if (!isClickInsideMenuButton && !isClickInsideMenu) {
			...
			document
				.querySelectorAll("[id^='employeeMenuButton-']")
				.forEach((button) => {
